...
export default function CodeBlock(props: { language?: string; code?: string }) {
  let [fullsized, setFullsized] = useState(false);
  ...
  let coes = code.split("\n");
  const maxCodeLine = 10;
  if (!fullsized && coes.length > maxCodeLine) {
    coes = coes.slice(0, maxCodeLine);
    showMoreBtn = true;
  }
  ...
  return (
    <>
      <div
        className={[styles.container, fullsized ? styles.full : ""].join(" ")}
      >
        <div className={styles.code}>
          <div className={styles.head}>
            <div className={styles.dots}>
              <span onClick={() => setFullsized(false)}>
                <svg
              ...
              <span onClick={() => setFullsized(true)}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 -960 960 960"
                >
          ...
          {showMoreBtn ? (
            <div className={styles.showmore} onClick={() => setFullsized(true)}>
              Show More
            </div>
          ) : null}
        </div>
      ...
      {fullsized ? (
        <div className={styles.bg} onClick={() => setFullsized(false)}></div>
      ) : null}
    </>
  );
}
